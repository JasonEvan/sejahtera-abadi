...
export async function GET(
  ...
  try {
    ...
    logger.info(
      `GET /api/beli/retur/${nomorNota} succeeded. Found ${data.length} items.`
    );
    ...
    logger.error(
      `GET /api/beli/retur/[nomornota] failed: ${
...
export async function PUT(
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const breturDeletePromise = tx.bretur.deleteMany({
        where: { nomor_nota: nomorNota },
      });

      await Promise.all([...oldStockUpdatePromises, breturDeletePromise]);

      // Update beli with the new data
      const beliUpdatePromises = validatedData.dataRetur.map((item) =>
        tx.beli.update({
          where: { id: item.id },
          data: {
            qty_barang: item.qty_barang,
    ...
    logger.info(
      `PUT /api/beli/retur/${nomorNota} succeeded. Retur data updated.`
    );
    return NextResponse.json({ message: "Retur updated successfully" });
  } catch (error) {
    logger.error(
      `PUT /api/beli/retur/[nomornota] failed: ${
        error instanceof Error ? error.message : error
      }`
    );
    return NextResponse.json(
