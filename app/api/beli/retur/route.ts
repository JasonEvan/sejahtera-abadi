...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await prisma.bretur.findMany({
      where: {
        nota: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const notaLama = await tx.bnota.findFirst({
        where: {
      ...
      for (const item of validatedData.dataNota) {
        if (item.retur_barang > 0) {
          // [3a] Dapatkan record beli asli
          const beliAsli = await tx.beli.findUnique({
            where: { id: item.id },
            select: { qty_barang: true, harga_barang: true },
          });
          ...
          const qtyBaru = beliAsli.qty_barang - item.retur_barang;

          await tx.beli.update({
            where: { id: item.id },
            data: {
              qty_barang: qtyBaru,
              total_harga: qtyBaru * beliAsli.harga_barang,
            },
          });

          // [3c] Buat record retur
          await tx.bretur.create({
            data: {
              nomor_nota: validatedData.nomorNota,
